...
public class Elevator extends FrameLayout {
    ...
    public Elevator(@NonNull Context context, int totalLayers, int currentLayer, int primaryColor, OnLayerSelectedListener listener) {
        this(context, totalLayers, currentLayer, primaryColor, null, listener);
    }

    public Elevator(@NonNull Context context, int totalLayers, int currentLayer, int primaryColor, int[] icons, OnLayerSelectedListener listener) {
        super(context);
    ...
    private void init() {
        ...
        float angleStep = this.totalLayers > 1 ? 90.0f / (this.totalLayers - 1) : 0.0f;
        ...
        for (int i = 0; i < this.totalLayers; i++) {
            ...
            double radians = Math.toRadians((i * angleStep) + 225.0f);
    ...
    public void updateSelectionByTouch(float relX, float relY) {
        ...
        if (minDistanceSq < dpToPx(80) * dpToPx(80) && closestLayer != this.selectedLayer) {
            ...
            if (this.listener != null) {
                this.listener.onLayerSelected(this.selectedLayer);
            }
        }
    }
